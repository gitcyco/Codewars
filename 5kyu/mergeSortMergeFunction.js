...
// It is much easier to combine two already-sorted lists into 1 sorted list than it is to sort a big list all at once)
...
const mergesorted = (left, right) => {
  ...
  for (let i = 0; i < len; i++) {
    if (idxL < left.length && idxR < right.length) {
      if (left[idxL] < right[idxR]) arr.push(left[idxL++]);
      else arr.push(right[idxR++]);
    } else if (idxL < left.length) arr.push(left[idxL++]);
    else if (idxR < right.length) arr.push(right[idxR++]);
  }
  return arr;
};
