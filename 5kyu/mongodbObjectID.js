...
// This field is unique and contains some information about the time the element was created, id of the process that created it, and so on.
...
// Note:
//
// If you take a close look at a Codewars URL, you will notice each kata's id
// (the XXX in http://www.codewars.com/dojo/katas/XXX/train/javascript) is really
// similar to MongoDB's ids, which brings us to the conjecture that this is the database powering Codewars.
// Examples
//
...
const Mongo = {
  isValid: function (s) {
    ...
    if (/[^0-9a-f]/g.test(s)) return false;
    return true;
  },
  getTimestamp: function (s) {
    if (!this.isValid(s)) return false;
    let msec = parseInt(s.slice(0, 8), 16) * 1000;
