...
// bonus to buy, and build, the largest three-dimensional beer can pyramid you can. 
...
export function beeramid(bonus: number, price: number): number {
  let beers: number = Math.floor(bonus / price);
  let level: number = 0;
  if (beers < 1) return 0;
  while (beers > 0) {
    level++
    beers = beers - (level * level);
  }
  return beers == 0 ? level : level - 1;
}