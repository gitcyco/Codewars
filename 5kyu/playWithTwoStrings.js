...
// Answer:
const getKey = (s) =>
  [...s].reduce((a, e) => (e in a ? (a[e] = !a[e]) : (a[e] = true), a), {});
const flip = (c) => (c.toLowerCase() === c ? c.toUpperCase() : c.toLowerCase());

function workOnStrings(a, b) {
  let aKey = getKey(a.toLowerCase());
  let bKey = getKey(b.toLowerCase());
  let bStr = "";
  for (let c of b) {
    if (aKey[c.toLowerCase()]) bStr += flip(c);
    else bStr += c;
  }
  let aStr = "";
  for (let c of a) {
    if (bKey[c.toLowerCase()]) aStr += flip(c);
    else aStr += c;
  }
  return aStr + bStr;
}
