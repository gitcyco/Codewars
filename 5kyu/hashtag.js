...
// They had this great idea that users should be able to specify relevant keywords to their posts using an ingenious
// idea by prefixing those keywords with the pound sign (#). Your job is to extract those keywords so that they can be used later on for whatever purposes.
//
// Note:
//
...
function getHashtags(post) {
  let arr = post.match(/(?:^|[\s])#+[a-z#]+/gi) || [];
  arr = arr.map((e) => e.trim().replace(/^#+/g, ""));
