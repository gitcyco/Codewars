...
// That idea is related with the purpose of this kata: we should select all the primes that when
// reversed are a different prime (so palindromic primes should be discarded).
//
// For example: 13, 17 are prime numbers and the reversed respectively are 31, 71 which are
// also primes, so 13 and 17 are "emirps". But primes 757, 787, 797 are palindromic
// primes, meaning that the reversed number is the same as the original, so they are not considered as "emirps" and should be discarded.
//
...
function findEmirp(n) {
  ...
  let sum = 0;
  for (let i = n - 1; i > 0; i--) {
    if (i in lookup && getRev(i) in lookup) {
      num++;
