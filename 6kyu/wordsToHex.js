...
// words and books would look like if they could be seen as images.
//
// For this task you are required to take a continuous string that can consist of any combination of words or characters and then
// convert the words that make up this string into hexadecimal values that could then be read as colour values.
//
...
//
// Each word will represent a hexadecimal value by taking the first three letters of each word and find
// the ASCII character code for each character. This will then give you one hexadecimal value that represents the colours red, green or blue.
...
function wordsToHex(words) {
  let ar = words.split(" ");
  let out = Array(ar.length).fill(0);
  out = out.map(
    (e, i) =>
      `#${ar[i][0] ? hex(asc(ar[i][0])) : "00"}${ar[i][1] ? hex(asc(ar[i][1])) : "00"}${
        ar[i][2] ? hex(asc(ar[i][2])) : "00"
      }`
  );
  return out;
}
