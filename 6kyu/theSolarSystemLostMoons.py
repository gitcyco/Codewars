# 6 kyu The Solar System - Lost Moons
# 
# 
...
# The planets should be ordered as per the order in the planet_moons dictionary, which is ["Mercury", "Venus", "Earth", "Mars", "Jupiter", "Saturn", "Uranus", "Neptune"].
# 
# If a moon is present but its planet is not, ignore it
# 
# The moons for each planet should be sorted and are then swept up by the planet on alternative sides. 
# The first moon goes on the right of the planet, the next one on the left, and so on.
# 
# For example, if "Saturn" is given in the planets, and the moons are ["Rhea", "Tethys", "Iapetus", "Dione"], the first moon (after sorting), "Dione", is put on the right. 
# The next moon, "Iapetus", now goes to the left of "Saturn". "Rhea" this time has to go to the right of "Saturn". 
# Last, "Tethys" goes to the left, because the moons have to be placed alternatively. 
# So "Saturn"'s orbit after collecting its lost planets would look like ["Tethys", "Iapetus", "Saturn", "Dione", "Rhea"].
# 
# EXAMPLE
# 
# If the planets are ["Venus", "Neptune", "Jupiter", "Earth", "Mars"] 
# and the moons are ["Proteus", "Rhea", "Io", "Moon", "Nereid", "Triton", "Phobos"], the Solar System you return should be:
# 
...
def lost_moons(planets, moons):
    ...
    for planet in planet_moons:
        if planet in planets:
            item = [planet]
            plist = planet_moons[planet]
            merged = list(set(plist).intersection(moons))
            merged.sort()
            for i, moon in enumerate(merged):
                if i % 2 == 0:
                    item.append(moon)
                else:
                    item.insert(0, moon)
