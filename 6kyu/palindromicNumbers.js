...
//
// You will be given an number as input and in the output you must return a string
// containing the number of iterations (i.e. additions) you had to perform to reach the palindromic
// result and the palindromic number itself, separated by a space. In Haskell return a tuple (Int, Integer).
//
// palindromize(195)
// 4 9339
//
...
// ###Some Assumptions
//
// You can assume that all numbers provided as input will be actually paired with a palindromic result
// and that you will reach that result in less than 1000 iterations and yield a palindrome.
//
// Answer:
function palindromize(num) {
  let i = 0;
  while (true) {
    if (checkPal(num)) return `${i} ${num}`;
    num = rev(num);
    i++;
  }
}

const rev = (n) => n + +n.toString().split("").reverse().join("");
const checkPal = (n) => n === +n.toString().split("").reverse().join("");
