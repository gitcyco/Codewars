// 6 kyu Santa's Master Plan
//
// Santa has just revealed his master plan! His goal was to automate
// as many of his tasks as possible by posting them as challenges in the #hackingholidays section of Codewars.
// Thanks to the solutions from these challenges, Santa has completed his tasks a week before Christmas!
...
function getAttendees(peopleInvited, responses) {
  return peopleInvited.filter((person) => {
    for (let res of responses) {
      if (res.name === person) return res.response === "accepted";
    }
    return true;
  });
}
