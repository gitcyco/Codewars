// 6 kyu Santa's Missing Gift List
//
...
//
// The integer for each child is such that the child should get the highest toy lower
// than or equal to that integer, and then, if there's more left, also get the highest
// toy lower than the rest and so on. Know that Santa never gives the same gift twice.
//
// For example, by Timmy's name is 160. This means that Timmy should get both
// a football and a teddy, because 128 + 32 = 160.
//
...
// Answer:
function gifts(n) {
  let num = 0b1000000000; // 200h === 512
  const bag = [];
  while (num > 0) {
    if (num & n) bag.push(GIFTS[num]);
    num >>= 1;
  }
