// 6 kyu Scheduling (Shortest Job First or SJF)
//
...
//
// SJF works by, well, letting the shortest jobs take the CPU first.
// If the jobs are the same size then it is First In First Out (FIFO).
// The idea is that the shorter jobs will finish quicker, so theoretically jobs won't get frozen because of large jobs.
...
function SJF(jobs, index) {
  ...
  let time = 0;
  for (let [item, idx] of map) {
    time += item;
    if (idx === index) return time;
  }
}
