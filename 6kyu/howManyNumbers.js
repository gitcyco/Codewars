...
//     Other numbers like 1456, 389, 157 don't belong to that group because in the first
//     number(1456), the difference between second and first digit 4 - 1 > 2; in the next one(389), we have 8 - 3 > 2;
...
// d5 - d4 <= d;
//
// The function should accept two arguments n and d; n is the upper limit of the range
// to work with(all the numbers should be less or equal than n), and d is maximum difference between every pair of its contiguous digits.
...
function selNumber(n, d) {
  ...
  let flag = true;
  for (let i = 10; i <= n; i++) {
    let digs = i.toString().split("").map(Number);
    for (let i = 1; i < digs.length; i++) {
      if (!(digs[i - 1] < digs[i]) || digs[i] - digs[i - 1] > d) flag = false;
    }
    if (flag) count++;
    flag = true;
  }
