...
//
// You will be given a meme (string), and your task is to identify its category, and send it
// to the right receiver: IT - 'Roma', chemistry - 'Maxim', design - 'Danik', or other - 'Vlad'.
//
...
const memesorting = (meme) => {
  const users = [
    new Meme("bug", "Roma"),
    new Meme("boom", "Maxim"),
    new Meme("edits", "Danik"),
  ];
  for (let char of meme) {
    for (let user of users) {
      if (user.check(char)) return user.name;
    }
  }
  return "Vlad";
};
