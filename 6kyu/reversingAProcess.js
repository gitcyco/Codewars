...
//
// Indeed it can happen that the decoding is impossible for strings composed of whatever letters
// from "a" to "z" when positive integer num has not been correctly chosen. In that case return "Impossible to decode".
// Examples
//
// decode "6015ekx" -> "mer"
...
function decode(r) {
  ...
  let num = (r.match(/\d+/) || [])[0];
  let str = (r.match(/[a-z]+/g) || [])[0];
  if (num === undefined || str === undefined || !checkNum(+num))
    return "Impossible to decode";
  let out = "";
  for (let c of str) {
    let char = rev(key.indexOf(c), +num);
    out += key[char];
  }
  return out;
}
