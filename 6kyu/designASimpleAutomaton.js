...
// Also make sure you set up the three states, q1, q2, and q3 for the myAutomaton instance. The test fixtures will be calling against myAutomaton.
// 
// As an aside, the automaton accepts an array of strings, rather than just numbers, or a number 
// represented as a string, because the language an automaton can accept isn't confined to just numbers. An automaton should be able to accept any 'symbol.'
// 
...
Automaton.prototype.readCommands = function (cmd) {
    this.states = [1, 0, 0];
    while (cmd.length > 0) {
        let cur = +cmd.shift();
        if (this.states[0]) {
