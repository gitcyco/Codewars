...
//
// It's your first day in the robot factory and your supervisor thinks that you should start with an easy task.
// So you are responsible for purchasing raw materials needed to produce the robots.
//
// A complete robot weights 50 kilogram. Iron is the only material needed to create a robot.
// All iron is inserted in the first machine; the output of this machine is the input for the next one, and so on.
// The whole process is sequential.
// Unfortunately not all machines are first class, so a given percentage of their inputs are destroyed during processing.
//
// Task
//
...
//
// Answer:
function calculateScrap(s, n) {
  return Math.ceil((n * 50) / s.reduce((a, e) => a * ((100 - e) / 100), 1));
}
