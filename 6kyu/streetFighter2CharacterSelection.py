...
# Test
# 
# For this easy version the fighters grid layout and the initial position will always 
# be the same in all tests, only the list of moves change.
# 
...
def street_fighter_selection(fighters, initial_position, moves):
    ...
    for move in moves:
        dir = dirs[move]
        y, x = [a + b for a, b in zip(cur, dir)]
        cur = [y, x % width] if y >= 0 and y < height else cur
        y, x = cur
        output.append(fighters[y][x])
    return output