...
//
//     The word starts with a consonant -> move consonants from the beginning of the word
...
function pigLatin(str) {
  if (/[^a-z]/gi.test(str)) return null;
  return str.toLowerCase().replace(/\w+/gi, (r) => {
    if (/^[^aeiou]/i.test(r[0])) {
      let cons = r.match(/^[^aeiou]+/)[0];
      r = r.replace(cons, "") + cons + "ay";
    } else {
      r = r + "way";
    }
    return r;
