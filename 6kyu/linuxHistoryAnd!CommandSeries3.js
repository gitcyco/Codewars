// 6 kyu Linux history and `!` command. Series#3 The `!-n` command
//
...
//   8  history
//
// and that should return the nth executed command line, for example with n=4 and the above history it should return touch me.
// If user ask for a n without any know entry for exampl n=12 here, the function should return !12: event not found.
//
...
function bangMinusN(n, history) {
  let items = history.trim().match(/\d+\s+.+(?=\n|$)/g);
  if (items.length - n < 0) return `!-${n}: event not found`;
  return items[items.length - n].replace(/\d+\s+/, "");
}
