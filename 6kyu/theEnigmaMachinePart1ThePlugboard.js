...
// for disguising the content of military communications.
// Alan Turing - the father of computing - formulated and developed concepts that are the basis
// of all computers in use today, he did this in response to the vital need to break those military communications.
// Turing and his colleagues at Bletchley Park are generally recognised as being responsible
// for shortening WWII by two years and saving an estimated 22 Million lives.
//
...
// Note
//
// In the actual usage of the original Enigma Machine, punctuation was encoded as words transmitted
// in the stream, in our code, anything that is not in the range A-Z will be returned unchanged.
// Kata
//
...
class Plugboard {
  constructor(wires = "") {
    if (wires.length % 2 !== 0 || wires.length / 2 > 10) throw new Error("!");
    const parts = wires.match(/../g) || [];
    const obj = {};
    for (const part of parts) {
      const [a, b] = part.split("");
      if (a in obj || b in obj) throw new Error("!");
      obj[a] = b;
      obj[b] = a;
    }
    this.wires = obj;
  ...
  process(wire) {
    if (wire in this.wires) return this.wires[wire];
    return wire;
  }
}
