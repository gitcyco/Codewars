...
// Answer:
function code(num) {
  return num.split("").reduce((str, e) => {
    let bin = (+e).toString(2);
    let bits = bin.length - 1;
    str += "0".repeat(bits) + "1" + bin;
    return str;
  }, "");
}
function decode(str) {
  ...
  while (index < str.length) {
    ...
    if (str[index] === "1") {
      ...
      throw new Error("Invalid format of input string, missing '1' sentinel");
    while (bits-- > 0) {
      dig += str[index++];
    }
    digits += parseInt(dig, 2);
  }
  return digits;
}
