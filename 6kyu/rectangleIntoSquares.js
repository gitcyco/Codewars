...
//
// You will return a collection or a string (depending on the language; Shell bash, PowerShell, Pascal and Fortran return a string)
...
// Notes:
//
//     lng == wdth as a starting case would be an entirely different problem and the drawing
//     is planned to be interpreted with lng != wdth.
...
//
// Answer:
function sqInRect(len, width, results = []) {
  let short = Math.min(len, width);
  let long = Math.max(len, width);
  if (short === long && results.length === 0) return null;
  if (short === 1) {
    return results.concat(new Array(long).fill(1));
  }
  results.push(short);
  long -= short;
  return long ? sqInRect(long, short, results) : results;
}
