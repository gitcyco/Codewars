...
// I have not translated this first one as usual because I did not solved it, and I fear not being able
// to solve it (Java is not my cup of... tea). @cjmcgraw, if you want to use my translation on your kata feel free to use it.
//
...
// should rotate the array to the left. If n is 0, then it should return the array unchanged.
//
// Example:
//
// var data = [1, 2, 3, 4, 5];
...
function rotate(array, n) {
  let rot = n % array.length;
  return array.slice(-rot).concat(array.slice(0, -rot));
}

// Super verbose:
function rotate_(array, n) {
  let rot = Math.abs(n) % array.length;
  if (n === 0 || rot === 0) return array;
  if (n < 0) {
    let beg = array.slice(rot);
    let end = array.slice(0, rot);
    return beg.concat(end);
  } else {
    let beg = array.slice(-rot);
    let end = array.slice(0, array.length - rot);
    return beg.concat(end);
  }
}
