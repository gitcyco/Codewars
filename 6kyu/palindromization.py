# 6 kyu Palindromization
# 
# You are given string elements and an int n. 
# Your task is to return a string that is a palindrome using elements from the string elements with the length n.
...
def palindromization(elements, n):
    if not elements or n < 2:
        return "Error!"
    outpal = elements[0] + elements[0]
    eindex = 1
    while len(outpal) < n:
        mid = ceil(len(outpal) / 2)
        left = outpal[0:mid]
        right = outpal[mid:]
        if len(outpal) % 2 == 0:
            outpal = left + elements[eindex] + right
            eindex = (eindex + 1) % len(elements)
        else:
            outpal = left + left[-1:] + right
    return outpal