...
// 
// starting_number = 20_000, reach = 2, target = 7_000_000_000
//
// Answer:
export function Mormons(num: number, rch: number, tgt: number, c: number = 0): number {
    return num >= tgt ? c : Mormons(num + (num * rch), rch, tgt, ++c);
}