...
//
// The name of the function with the corresponding order of its arguments
// and the data structure for the output is as it follows below:
//
...
//
// Answer:
function findArr(arrA, arrB, [low, high], wanted) {
  const aObj = arrA.reduce((a, e) => {
    if (e < low || e > high) return a;
    if ((wanted === "odd" && e % 2) || (wanted === "even" && e % 2 === 0))
      a[e] = (a[e] || 0) + 1;
    return a;
  }, {});
  const bObj = arrB.reduce((a, e) => ((a[e] = (a[e] || 0) + 1), a), {});
  return [...new Set(arrB)]
    .filter((e) => e in aObj && aObj[e] > 1 && bObj[e] > 1)
    .sort((a, b) => a - b);
}
