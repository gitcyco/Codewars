// 6 kyu T.T.T.51: Four warriors and a lamp
//
// There are four warriors, they need to go through a dark tunnel.
// Tunnel is very narrow, every time only can let two warriors through, and there are lot of dangerous traps.
// Fortunately, they have a lamp that can illuminate the tunnel to avoid being trapped by the trap.
//
...
// For example:
//
// Four warriors is a,b,c,d. Their speed are [3,4,5,6].
// It means they need 3 minutes, 4 minutes, 5 minutes and 6 minutes to cross the tunnel.
...
// Task
//
// Complete function shortestTime() that accepts 1 argument: speed that are the spent time of four warriors.
// Returns the shortest time that all warriors go through the tunnel.
//
...
function shortestTime(speed) {
  const [a, b, c, d] = speed.slice().sort((a, b) => a - b);
  return Math.min(b + a + c + a + d, b + a + d + b + b);
}
