...
// 
// So this function should return the first pair of the two prime numbers spaced with a step of g between the limits m, n if these g-steps prime numbers 
// exist otherwise nil or null or None or Nothing or [] or "0, 0" or {0, 0} or 0 0 or "" (depending on the language).
// Examples:
// 
...
// Answer:
function step(g, m, n) {
    const arr = primeSeries(m, n);
    for (let i = 0; i < arr.length; i++) {
        if (arr.includes(arr[i] + g)) return [arr[i], arr[i] + g]
    }
    return null;
}
