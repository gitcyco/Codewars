...
function sortSequence(sequence) {
  let newArr = sequence
    ...
    .sort(
      (a, b) =>
        a.reduce((ac, e) => (ac += e), 0) - b.reduce((ac, e) => (ac += e), 0)
    );
...
const sometimes = function (fn) {
  let count = 0;
  return (...rest) =>
    ++count % 2 == 0 && count > 3 ? "hmm, I don't know!" : fn(...rest);
};
...
function convert(input, source, target) {
  return base10toBaseX(
    target.length,
    target,
    baseXtoBase10(source.length, source, input)
  );
}
...
function baseXtoBase10(base, source, val) {
  ...
  return val
    .split("")
    .reverse()
    .reduce((acc, e, i) => (acc += key.indexOf(e) * source.length ** i), 0);
}
...
function shortesttoChar(s, c) {
  ...
  return s
    .split("")
    .map(
      (e, i) => ind.map((val) => Math.abs(i - val)).sort((a, b) => a - b)[0]
    );
}
...
function bitsWar(numbers) {
  ...
  let oddWins = odds.reduce(
    (acc, e) => (acc += e > 0 ? countBits(e) : -countBits(e)),
    0
  );
  let evenWins = evens.reduce(
    (acc, e) => (acc += e > 0 ? countBits(e) : -countBits(e)),
    0
  );
...
function grille(message, code) {
  ...
  return message
    .padStart(binStr.length, " ")
    .split("")
    .filter((e, i) => binStr[i] == 1 && e != " ")
    .join("");
...
function chmodCalculator(perm) {
  ...
  return ["user", "group", "other"].reduce(
    (acc, e) => (acc += getVal(e, perm, vals)),
    ""
  );
  //return `${u}${g}${o}`;
}

const getVal = (id, perm, vals) =>
  perm[id] != undefined
    ? perm[id].split("").reduce((acc, e) => (acc += vals[e]), 0)
    : 0;
...
const count = (string) =>
  string
    .split("")
    .reduce((obj, val) => ((obj[val] = obj[val] + 1 || 1) ? obj : obj), {});
...
const solution = (num) =>
  num > 0
    ? [...Array(num)]
        .map((e, i) => (e = i))
        .reduce((a, e) => (a += e % 3 === 0 || e % 5 === 0 ? e : 0), 0)
    : 0;
...
function changer(str) {
  ...
  for (let i = 0; i < oldStr.length; i++) {
    ...
    newStr +=
      vowels.indexOf(char) != -1 ? char.toUpperCase() : char.toLowerCase();
  }
  return newStr;
}
...
function isOddHeavy(n) {
  return n.filter((e) => e % 2).sort()[0] <
    n.filter((e) => !(e % 2)).sort((a, b) => b - a)[0] ||
    n.filter((e) => e % 2).sort().length == 0
    ? false
    : true;
}
...
function to_nato(words) {
  ...
  console.log(wordArr);
  //wordArr.map
  const natoArray = wordArr.map((e, i, a) =>
    nato[e.toUpperCase()]
      ? a[i + 1] == " "
        ? nato[e.toUpperCase()].trim()
        : nato[e.toUpperCase()]
