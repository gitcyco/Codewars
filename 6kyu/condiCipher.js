...
//     Encode the first letter of your message by moving 10 places to the right from the letter's position in the key alphabet.
//     If the first letter were say o then the letter 10 places to the right in the key alphabet is j, so o would be encoded as j.
...
//     Use the position of the previous plaintext letter as the number of places to move to encode the next plaintext number.
//     If you have just encoded an o (position 6) , and you now want to encode say n, then you move 6 places to the right from n which brings you to x.
...
const genKey = (k) => [...new Set([...k, ..."abcdefghijklmnopqrstuvwxyz"])];
const shR = (s, n) => (s + n) % 26;
const shL = (s, n) => (s + 26 - n) % 26;

function encode(message, key, initShift) {
  const keySet = genKey(key);
  let offset = initShift % 26;
  let output = "";
  for (let char of message) {
    let s = keySet.indexOf(char);
    if (s === -1) output += char;
    else {
      let idx = shR(s, offset);
      output += keySet[idx];
      offset = s + 1;
    }
  }
