...
// She wants you to build a system that will edit out all known acronyms or else will notify the sender if unknown acronyms are present.
//
...
function acronymBuster(string) {
  ...
  let acr = string.match(/[A-Z]{3,}/g) || [];
  for (let m of acr) {
    if (!key[m])
      return `${m} is an acronym. I do not like acronyms. Please remove them from your email.`;
    string = string.replace(m, key[m]);
  }
  return string.replace(/([^.]+)/gi, (e) =>
    e.replace(/(^|\s)[a-z]/i, (r) => r.toUpperCase())
  );
}
