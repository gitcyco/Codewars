...
function bitsBattle(nums) {
  let evens = nums.filter((e) => e % 2 === 0 && e).reduce((ac, e) => ac + eC(e), 0);
  let odds = nums.filter((e) => e % 2 !== 0 && e).reduce((ac, e) => ac + oC(e), 0);
  return evens === odds ? "tie" : evens > odds ? "evens win" : "odds win";
}

const oC = (n) => (n.toString(2).match(/1/g) || []).length;
const eC = (n) => (n.toString(2).match(/0/g) || []).length;
