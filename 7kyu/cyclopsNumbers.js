...
// 101
// 11111111011111111
// You must take an input, n, that will be in decimal format (base 10), then return True if that
// number wil be a cyclops number when converted to binary, or False if it won't.
...
function cyclops(n) {
  ...
  if ((bin.match(/0/g) || []).length !== 1) return false;
  if (bin.indexOf("0") === Math.floor(bin.length / 2)) return true;
  return false;
}
