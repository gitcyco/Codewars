...
//
// Your company MRE Tech has hired a spiritual consultant who advised on a new 
// Balance policy: Don't take sides, don't favour, stay in the middle. 
// This policy even applies to the software where all strings should now be centered. 
// You are the poor soul to implement it.
// Task
// 
// Implement a function center that takes a string strng, an integer width, and an optional 
// character fill (default: ' ') and returns a new string of length width where 
...
//
// Answer:
function center (str, w, fill = ' ') {
    let len = w - str.length;
    return len < 0 ? str : fill.repeat(Math.ceil(len / 2)) + str + fill.repeat(Math.floor(len / 2));
}