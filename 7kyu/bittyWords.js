// 7 kyu Bitty Words
//
// Your friend has given you a secret message nested inside a block of text.
// She provided you the key to her code as a number and now you must decode the message.
//
// The number, in binary, represents on/off flags for each word of the phrase.
// The bit order, overlayed on the array of words, will give which words to include in the phrase.
//
// Example:
...
//
// Answer:
function bittyWords(w, bits) {
  const sArr = w.split(" ");
  const bArr = bits.toString(2).padStart(sArr.length, 0).split("");
  return sArr.filter((e, i) => bArr[i] != 0).join(" ");
}
