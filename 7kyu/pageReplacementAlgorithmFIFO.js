// 7 kyu Page replacement algorithms: FIFO
//
// In a computer operating system that uses paging for virtual memory management, page replacement algorithms
// decide which memory pages to page out when a page of memory needs to be allocated. Page replacement happens
// when a requested page is not in memory (page fault) and a free page cannot be used to satisfy
// the allocation, either because there are none, or because the number of free pages is lower than some threshold.
// The FIFO page replacement algorithm
//
// The first-in, first-out (FIFO) page replacement algorithm is a low-overhead algorithm that requires little
// bookkeeping on the part of the operating system. The idea is obvious from the name: the operating system
// keeps track of all the pages in memory in a queue, with the most recent arrival at the back, and the oldest
// arrival in front. When a page needs to be replaced, the oldest page is selected.
...
// as input: the number of maximum pages that can be kept in memory at the same time n and a reference
// list containing numbers. Every number represents a page request for a specific page (you can see this number as the unique ID of a page).
...
function fifo(n, ref) {
  ...
  for (let i = 0; i < ref.length; i++) {
    if (arr.includes(ref[i])) continue;
    arr[head++] = ref[i];
    if (head === n) head = 0;
  }
  return arr;
