...
//
// Answer:
function hasSurvived(att, def) {
  let ap = att.reduce((a, e) => a + e, 0);
  let dp = def.reduce((a, e) => a + e, 0);
  let aw = 0;
  let dw = 0;
  let len = Math.max(att.length, def.length);
  for (let i = 0; i < len; i++) {
    if (att[i] > def[i] || def[i] === undefined) aw++;
    if (def[i] > att[i] || att[i] === undefined) dw++;
  }
  if (aw > dw) return false;
  if (aw === dw && ap > dp) return false;
  return true;
}
