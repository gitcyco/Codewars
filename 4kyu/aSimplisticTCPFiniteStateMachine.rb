# 4 kyu A Simplistic TCP Finite State Machine (FSM)
# 
# Automatons, or Finite State Machines (FSM), are extremely useful to programmers 
...
def traverse_TCP_states(events)
  ...
  machine = getfsm()
  for event in events
    if (machine[state.to_sym] && machine[state.to_sym][event.to_sym])
        state = machine[state.to_sym][event.to_sym]
    else
      return "ERROR"
    end
  end
