...
class User {
  constructor() {
    this._r = -8;
    this._p = 0;
    this._a = [-8, -7, -6, -5, -4, -3, -2, -1, 1, 2, 3, 4, 5, 6, 7, 8];
  }
  get progress() {
    return this._p;
  }
  ...
  incProgress(val) {
    if (!this._a.includes(val)) throw new Error("Invalid value");
    let dif = this._a.indexOf(val) - this._a.indexOf(this._r);
    if (dif === 0) this._p += 3;
    else if (dif === -1) this._p += 1;
    else if (dif > 0) this._p += 10 * dif * dif;
    if (this._p > 100) {
      while (this._p > 100) {
        this._r = this._r + 1 === 0 ? 1 : this._r + 1;
        this._p -= 100;
      }
    }
    if (this._r >= 8) {
