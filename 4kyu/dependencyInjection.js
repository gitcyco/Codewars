...
//
// Did you hear about Dependency Injection pattern? The main idea of this pattern
// is that you may have ability to pass dependencies into your function in any order and they will be resolved automatically. Take a look at this:
//
...
// Let's assume that all dependencies are stored in the single hash object (for instanse 'deps')
// and the injector function will be sought among 'deps' properties:
//
// var deps = {
...
// And don't forget about nested functions. You shouldn't handle them.
//
// Answer:
/**
...
DI.prototype.inject = function (func) {
  let params = func.toString().match(/\(([\s\S]*?)\)/) || [];
  if (params) {
    let pnames = params[1].split(",");
    if (pnames[0] !== "") {
      pnames = pnames.map((e) => this.dependency[e.trim()]);
    } else pnames = [];
    return function () {
      return func(...pnames);
    };
