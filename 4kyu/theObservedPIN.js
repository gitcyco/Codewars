...
// We could name the function getPINs (get_pins in python, GetPINs in C#). But please note that all PINs, the observed
// one and also the results, must be strings, because of potentially leading '0's. We already prepared some test cases for you.
//
...
function getPINs(obs) {
  ...
  const getRest = (chars, out = []) => {
    let start = chars[0];
    let rest = chars.slice(1);
    let vals = lookup[start].map((e) => e.toString());
    if (rest.length) {
      let items = getRest(rest);
      for (let val of vals) {
  ...
  return [...new Set(getRest(obs.split("")))];
}
