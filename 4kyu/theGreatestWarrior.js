...
//     "Easy fight", "A good fight", "An intense fight". Return "Easy fight" if your warrior is 2 or more levels higher than your enemy's level.
...
//     Training will accept an array of three elements (except in java where you'll get 3 separated arguments): the description, the experience points
//     your warrior earns, and the minimum level requirement.
//     If the warrior's level meets the minimum level requirement, the warrior will receive the experience points from it and store the description of the training.
//     It should end up returning that description as well.
...
class Warrior {
  ...
  training(arr) {
    ...
    this.#listOfAchievements.push(type);
  ...
  #setExperience(exp) {
    if (exp > 0) {
      this.#currentExp += exp;
      if (this.#currentExp > 10000) this.#currentExp = 10000;
      this.#currentLevel = Math.floor(this.#currentExp / 100);
      this.#currentRank = this.#getRank(this.#currentLevel);
    }
  }
  #getRank(level) {
  ...
  achievements() {
    return this.#listOfAchievements;
  }
}
