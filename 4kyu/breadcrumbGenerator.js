...
// For this kata, your purpose is to create a function that takes a url, strips
// the first part (labelling it always HOME) and then builds it making each element
// but the last a <a> element linking to the relevant path; last has to be a <span> element getting the active class.
//
// All elements need to be turned to uppercase and separated by a separator, given as the second parameter
// of the function; the last element can terminate in some common extension like .html, .htm, .php or .asp;
// if the name of the last element is index.something, you treat it
// as if it wasn't there, sending users automatically to the upper level folder.
//
...
// (other than the root/home) is longer than 30 characters, you have to shorten it, acronymizing it
// (i.e.: taking just the initials of every word); url will be always given
// in the format this-is-an-element-of-the-url and you should ignore words in this array
// while acronymizing: ["the","of","in","from","by","with","and", "or", "for", "to", "at", "a"];
...
function generateBC(url, separator) {
  let parts = url
    .replace(/^https?:\/\/|[#?].*|\.html|\.htm|\.php|\.asp|index/gi, "")
    .split("/");
  let fullPath = "";
  return parts
    .filter((e) => e[0])
    .map((part, i, arr) => {
      let path = i === 0 ? "" : part;
      let tag = i === 0 ? "HOME" : shorten(part);
      fullPath += `${path}/`;
      if (i === arr.length - 1) return `<span class="active">${tag}</span>`;
      return `<a href="${fullPath}">${tag}</a>`;
    })
    .join(`${separator}`);
}
function shorten(str) {
  ...
  if (str.length > 30) {
    let arr = str.split("-").filter((e) => !elim.includes(e.toLowerCase()));
    return arr.map((e) => e[0].toUpperCase()).join("");
  }
