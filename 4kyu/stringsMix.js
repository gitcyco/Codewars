...
function mix(s1, s2) {
  let result = [];
  let keySet = new Set();
  const s1Obj = count(s1.replace(/[^a-z]/g, ""));
  const s2Obj = count(s2.replace(/[^a-z]/g, ""));
  for (let k in s1Obj) if (s1Obj[k] > 1) keySet.add(k);
  for (let k in s2Obj) if (s2Obj[k] > 1) keySet.add(k);
  for (let k of keySet) {
    if (s1Obj[k] === undefined || s2Obj[k] > s1Obj[k]) {
      result.push(`2:${k.repeat(s2Obj[k])}/`);
      continue;
    }
    if (s2Obj[k] === undefined || s1Obj[k] > s2Obj[k]) {
      result.push(`1:${k.repeat(s1Obj[k])}/`);
      continue;
    }
    if (s1Obj[k] == s2Obj[k]) result.push(`=:${k.repeat(s1Obj[k])}/`);
  }
  return result
    .sort()
