...
function boolfuck(code, input = "") {
    ...
    if (tmp.length > 0) outChars += String.fromCodePoint(parseInt(tmp.reverse().join('').padStart(8, 0), 2));
    return outChars;
}


function* getBit(input) {
    let chars = input.split('');
    let bits = [];
    for (let i = 0; i < chars.length; i++) {
        bits = chars[i].charCodeAt().toString(2).padStart(8, 0).split('').reverse();
